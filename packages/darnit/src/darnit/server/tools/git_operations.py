"""Git operation tools for OpenSSF Baseline remediation workflow.
    ...
    try:
        ...
        if not message:
            ...
            if file_descriptions:
                ...
                message = f"chore(security): add {items} for OpenSSF Baseline compliance"
            else:
                message = "chore(security): apply OpenSSF Baseline remediations"

        # Add trailer
        full_message = f"""{message}

Applied via OpenSSF Baseline MCP server.
...
"""
        ...
        if not title:
            title = "chore(security): OpenSSF Baseline compliance improvements"

        # Generate body if not provided
        if not body:
            body = """## Summary

This PR addresses OpenSSF Baseline compliance requirements.
...
These changes help satisfy requirements from the [OpenSSF Baseline](https://baseline.openssf.org/).

## Testing

- [ ] Reviewed changes for accuracy
- [ ] Verified no sensitive information exposed
- [ ] Re-ran OpenSSF Baseline audit to confirm improvements

---
*Generated by OpenSSF Baseline MCP server*
"""
