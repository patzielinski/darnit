...
SENSITIVE_DATA_PATTERNS: dict[str, dict[str, Any]] = {
    "pii": {
        "patterns": [
            r"""(?:^|[\s({\[,])"?(email|e_mail|user_email)"?\s*[:=]""",
            r"""(?:^|[\s({\[,])"?(phone|phone_number|mobile|telephone)"?\s*[:=]""",
            r"""(?:^|[\s({\[,])"?(ssn|social_security|social_security_number)"?\s*[:=]""",
            r"""(?:^|[\s({\[,])"?(date_of_birth|dob|birth_date|birthday)"?\s*[:=]""",
            r"""(?:^|[\s({\[,])"?(street_address|home_address|mailing_address)"?\s*[:=]""",
            r"""(?:^|[\s({\[,])"?(first_name|last_name|full_name)"?\s*[:=]""",
            r"""(?:^|[\s({\[,])"?(passport|passport_number)"?\s*[:=]""",
            r"""(?:^|[\s({\[,])"?(driver_license|drivers_license)"?\s*[:=]""",
            r"""(?:^|[\s({\[,])"?(national_id|id_number)"?\s*[:=]""",
        ],
...
INJECTION_PATTERNS: dict[str, dict[str, Any]] = {
    ...
    "ssrf": {
        "patterns": [
            ...
            r"axios\s*\(.*req\.",
            r"http\.get\s*\(.*req\.",
            r"requests\.(?:get|post|put|delete|head|patch)\s*\(.*request\.",
            r"urllib\.request\.urlopen\s*\(.*(?:request\.|input\(|argv|params|args)",
        ],
        "severity": "high",
        "cwe": "CWE-918",
